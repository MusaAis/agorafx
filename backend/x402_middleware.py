...
def _cfg(
    ...
    price_f = price_usdc if price_usdc is not None else float(os.getenv("X402_SIGNAL_PRICE_USDC", "0.001"))
    units   = int(price_f * 1_000_000)
    backend = os.getenv("BACKEND_URL", "https://api.kudiarc.xyz").rstrip("/")
    resource = resource_url if resource_url is not None else backend + "/rates/signal"
    return {
...
def make_x402_guard(
    ...
    Usage — fixed price (backwards compatible, signal endpoint):
        payer: str = Depends(require_x402)

    Usage — custom price per endpoint (article paywall):
        payer: str = Depends(make_x402_guard(
            price_usdc=0.05,
            resource_url="https://api.kudiarc.xyz/intelligence/{id}",
        ))
    """
    async def _guard(request: Request) -> str:
        _resource = resource_url or (
            os.getenv("BACKEND_URL", "https://api.kudiarc.xyz").rstrip("/")
            + str(request.url.path)
        )
        cfg = _cfg(price_usdc=price_usdc, resource_url=_resource)
...
# Existing Depends(require_x402) on /rates/signal continues to work unchanged.
require_x402 = make_x402_guard()
