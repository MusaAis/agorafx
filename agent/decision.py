...
v3 changes (fixes EURC positional bias bug found June 29):
...
- NGN: round to nearest whole number. Example: ₦1,371 not ₦1371.06316
- KES/GHS/ZAR/EGP: round to 2 decimal places. Example: ₵11.57 not ₵11 or ₵11.5740
...
def _clean_question(decision: dict) -> dict:
    ...
    if pair == "USDC/EURC":
        ...
        if rate >= 1000:
            ...
            rate_r = round(rate, 2)
            shown  = f"{rate_r:.2f}"
        decision["question"]  = f"Will 1 USDC be worth {direction} {sym}{shown} in the next hour?"
        decision["threshold"] = int(round(rate_r * 1_000_000))
    return decision
    ...
    for _ in range(len(keys)):
        try:
            ...
            if "rate_limit" in str(e).lower() or "429" in str(e) or "401" in str(e):
                log.warning("Groq key rejected or limited, rotating to next key")
                continue
...
async def build_scheduled_market() -> dict | None:
    ...
    for p in MONITORED_PAIRS:
        ...
        if pair == "USDC/EURC":
            ...
            rate_r = round(rate, 2)
        threshold = int(round(rate_r * 1_000_000))
        ...
        if pair == "USDC/EURC":
            ...
            shown = f"{rate_r:,}" if rate >= 1000 else f"{rate_r:.2f}"
            question = f"Will 1 USDC be worth {dw} {sym}{shown} in the next hour?"
